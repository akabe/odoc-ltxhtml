...
let dft_aux flag xl =
  ...
  let twf t k = exp {re = 0.0; im = c *. float (t * k)} in (* twiddle factor *)
  let rec one_to_n acc i =
    if i = -1 then acc else one_to_n (i :: acc) (i - 1) in
  let nl = one_to_n [] (n - 1) in (* = [0; 1; 2; ...; n-1] *)
  let calc_sum k =
    let f acc t x = add acc (mul x (twf t k)) in
    List.fold_left2 f zero nl xl in
...
let fft_aux flag xl =
  ...
  let twf m n = exp {re=0.; im=c *. float m /. float n} in (* twiddle factor *)
  let rec sep_list le lo = function
    | xe::xo::l -> sep_list (xe :: le) (xo :: lo) l
    | [] -> (List.rev le, List.rev lo)
    | _ -> failwith "# of data points must be 2^N"
  in
  let rec loop n = function
    | [xe; xo] -> [add xe (mul xo (twf 0 2)); add xe (mul xo (twf 1 2))]
    | xl ->
       let n' = n / 2 in
